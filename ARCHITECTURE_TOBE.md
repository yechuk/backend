...
이 Django 애플리케이션은 General Manager(단장)가 선수 영입, 유지, 방출과 같은 로스터 의사결정을 지원하도록 설계된 데이터 기반 야구 운영 플랫폼이다. 시스템은 MLB 선수 시즌 기록과 계약 데이터를 통합해 미래 성적을 예측하고, 선수의 적정 가치를 산출하며, 이를 선수 상세 분석·시뮬레이션·계약 비교 화면에 연결해 구단 관점의 의사결정을 보조한다.
...
flowchart TB
    ...
    subgraph sources [Data Sources]
        ...
        GS1["Google Sheets<br/>COT's Contracts Export"]
        GS2["Google Sheets<br/>Salary Data Export"]
    end
    ...
    subgraph ingestion [Ingestion]
        ...
        CONTRACTS["Contracts Import/Normalize (Google Sheet → DB)"]
        SALARY["Salary Import/Normalize (Google Sheet → DB)"]
    end
...
sequenceDiagram
    ...
    participant GS1 as COT Google Sheet
    participant GS2 as Salary Google Sheet
    ...
    M->>W: Upload contract/AAV spreadsheet
    W->>DB: Normalize and store compensation/AAV
    DB-->>W: OK

    M->>W: Upload salary spreadsheet
    W->>DB: Normalize and store salary
    DB-->>W: OK

    M->>W: View player detail
    W->>DB: Query player profile + cached predictions
    DB-->>W: Player data + predicted_value + predicted_aav
    W->>API: Fetch player stats (by id/season)
    API-->>W: Stats payload (JSON)
    W->>DB: Cache/store stats if needed
    DB-->>W: OK
    W-->>M: Render stats + value + AAV
```
...
2. **팀 데이터셋(XLSX)** — 팀 수준 데이터셋은 XLSX 파일로 관리한다. 사용자는 XLSX를 업로드하고, 시스템은 이를 파싱하여 데이터베이스에 적재한다(배치 임포트). 이 데이터는 팀 승률/수익 모델의 입력(또는 파생 변수 생성)의 기준이 된다.

3. **선수 스탯(API)** — 선수 시즌별 스탯은 외부 API에서 조회해 가져온다. 현재 기준 기본 후보는 **MLB Stats API**이며, Python 연동 시에는 `MLB-StatsAPI` 패키지(`statsapi`)를 우선 검토한다. 웹 요청 시 API를 호출해 최신 스탯을 수신하고, 조회 비용/속도를 위해 결과를 DB에 캐시(또는 스냅샷 저장)할 수 있다. 초기 구현은 `lookup_player`, `player_stat_data`, `schedule` 같은 고수준 함수 중심으로 시작하고, 필요한 경우 `get`으로 raw JSON 엔드포인트를 직접 호출한다.
...
본 프로젝트는 시장 수익을 직접 환산하는 방식 대신, 선수의 과거 시즌 기록과 세이버메트릭스를 바탕으로 **미래 성적을 예측**하고 이를 계약 정보와 함께 해석하는 구조를 사용한다. 문서 내 "적정 가치(predicted_value)"와 `predicted_aav`는 단일 수익 공식이 아니라, 미래 성적 전망과 비교 기준을 종합한 의사결정용 지표로 다룬다.
...
flowchart LR
    subgraph train [Offline Training]
        DB[(DB)] --> FE[Feature Engineering]
        DB --> Market[Normalized Contract Market Data]
    ...
    subgraph infer [Online Inference]
        OnlineDB[(Player/Season DB)] --> RuntimeFE[Inference Feature Build]
...
`6.4.2`와 `6.7`에서 정의한 모델은 **오프라인 학습 단계**와 **온라인 추론 단계**를 구분한다. Google Sheets에서 업로드한 `COT's Contracts` 기반 계약 데이터는 DB에 정규화 저장된 뒤 **오프라인에서 AAV 모델 학습용 타깃/시장 기준 데이터**로 사용된다. 실제 상세 페이지 조회 시에는 선수 시즌 기록으로부터 미래 성적을 추론하고, 그 예측 결과를 이미 학습된 AAV 모델에 넣어 `predicted_aav`와 계약 의사결정 지표를 산출한다.

### 5.2. 입력 데이터

- **시즌별 선수 기록**: 타자/투수 기본 기록과 세이버메트릭스
...
선수 예측 모델 학습 및 서비스 반영에 필요한 데이터셋 구조는 다음과 같다.

#### 5.6.1. 선수 시즌 데이터 (Player-level)

상세 필드는 `6.4.1. MLBPlayerSeason` 정의를 기준으로 관리한다.

- **타자**: AB, hits, HR, RBI, AVG, OBP, SLG, OPS, WAR, wOBA, wRC+, BABIP, OPS+
- **투수**: IP, ERA, SO, BB, WHIP, WAR, FIP, xFIP, K/9, BB/9
- **WAR 계산/검증용 원천 변수**
...
- **Phase 2**: 피처 엔지니어링 및 베이스라인 예측 모델 구축
...
- **수집 방식**: (1) 파일 기반 배치 임포트, (2) 선수 스탯은 `statsapi` 기반 API 호출로 온디맨드 조회(필요 시 캐시), (3) 계약/AAV 데이터는 사용자가 Google Sheets에서 다운로드한 `COT's Contracts` 스프레드시트를 업로드하고 이를 정규화해 적재, (4) salary 데이터도 별도 Google Sheets 스프레드시트를 업로드해 정규화 적재
...
WAR(Win Above Replacement)는 대체 선수 대비 팀 승리에 얼마나 더 기여했는지를 나타내는 누적 가치 지표다. 본 프로젝트에서는 WAR를 예측/표시할 때 아래 구조를 기준 개념으로 사용한다.
...
$$
\text{WAR} = \left( \left( \frac{\text{League FIP} - \text{Pitcher FIP}}{\text{Pitcher Specific Runs Per Win}} + \text{Replacement Level} \right) \times \frac{\text{IP}}{9} \right) \times \text{Leverage Multiplier for Relievers} + \text{League Correction}
$$

- **해석 메모**
  - 타자 WAR는 공격, 주루, 수비, 포지션 보정, 리그 보정, 대체 선수 대비 기여를 종합한다.
  - 투수 WAR는 FIP 기반 기여, 이닝 규모, 불펜 레버리지, 리그 보정을 함께 반영한다.
  - 실제 구현 시 데이터 소스에 따라 세부 계수와 보정 방식은 달라질 수 있으므로, 원천 데이터 정의와 일치하도록 정규화한다.
  - 따라서 WAR를 단순 결과값으로만 저장하지 않고, 가능하면 구성 요소 변수도 함께 저장하거나 재현 가능한 형태로 확보해야 한다.
...
- **역할**: 선수별로 t+1, t+2, … t+N 시즌에 대한 주요 성적 지표(타자: AVG, OPS, HR, WAR, wOBA, wRC+ 등 / 투수: ERA, FIP, WAR 등) 전망과, 이를 바탕으로 한 예상 AAV를 산출한다.
...
- **목표**: 미래 성적 예측 결과와 시장 계약 데이터를 바탕으로 선수의 예상 AAV를 추정한다.
...
- **성능 비교**: 검증/테스트 세트에서 성적 예측 모델의 정확도를 비교하여, 더 성능이 좋은 모델을 단일 모델로 사용할 수 있음
...
> **연구 참고 방향**: 최근 야구 성적 예측 연구에서는 `과거 2~4시즌의 다변량 입력`, `sliding window 기반 다음 시즌 예측`, `RMSE/MAE/MAPE 기반 비교`, `SHAP를 통한 해석`이 유효한 설계로 제시된다. 특히 Sun et al. (2022)의 LSTM 기반 MLB 홈런 예측은 `시즌 시계열 + LSTM`이 실용적인 베이스라인이 될 수 있음을 보여준다. 본 프로젝트는 이를 참고하되, 단일 타깃 예측에 머물지 않고 타자/투수 다지표 예측과 장기 성과 해석으로 확장한다.
